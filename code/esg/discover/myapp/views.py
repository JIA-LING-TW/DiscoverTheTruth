...
def ESGEachCompany(request):
    ...
    if selected_model:
        ...
        # 提取字段名稱和顯示名稱
        fields = [
            (field.name, field.verbose_name) for field in selected_model._meta.fields
        ]
...
def login(request):
    ...
    return render(
        request,
        "login.html",
        {"captcha": captcha, "message": messages.get_messages(request)},
    )
...
def register(request):
    if request.method == "POST":
        ...
        # 建立新使用者
        User.objects.create(
            username=username, email=email, password=make_password(password)  # 加密密碼
        )
...
def upload_water_resource_management_data(request):
    ...
    file_path = os.path.abspath(
        os.path.join(
            ...
            "..",
            "2021-2023_ESG(E)",
            "water_resource_management.xlsx",
        )
    )
    ...
    try:
        ...
        if not all(column in df.columns for column in required_columns):
            return JsonResponse(
                {"error": "Excel 檔案格式錯誤，缺少必要欄位。"}, status=400
            )
        ...
        for _, row in df.iterrows():
            # 檢查資料是否已存在
            if WaterResourceManagement.objects.filter(
                year=row["年份"], company_code=row["公司代號"]
            ).exists():
...
def upload_waste_management_data(request):
    ...
    try:
        ...
        for _, row in df.iterrows():
            # 檢查資料是否已存在
            if WasteManagement.objects.filter(
                year=row["年份"], company_code=row["公司代號"]
            ).exists():
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if EnergyManagement.objects.filter(
                ...
                print(
                    f"重複資料跳過：{row['年份']} - {row['公司代號']} - {row['公司名稱']}"
                )
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            GreenhouseGasEmission.objects.create(
                ...
                emission_intensity=row.get(
                    "溫室氣體排放密集度-密集度(噸CO2e/單位)", None
                ),
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            CompanyBoard.objects.create(
                ...
                training_hours_compliance_rate=row.get(
                    "董事進修時數符合進修要點比率", 0
                ),
            )
...
def upload_function_committee_data(request):
    ...
    try:
        ...
        for _, row in df.iterrows():
            # 檢查資料是否已存在
            if CorporateGovernance.objects.filter(
                year=row["年份"], company_code=row["公司代號"]
            ).exists():
                continue  # 如果資料已存在，跳過此行
...
def upload_employee_develop_data(request):
    ...
    file_path = os.path.abspath(
        os.path.join(settings.BASE_DIR, "..", "2021-2023_ESG(S)", "hr_develop.xlsx")
    )
    ...
    try:
        ...
        required_columns = [
            ...
            "公司代號",
            "公司名稱",
            "員工福利平均數(仟元/人)(每年6/2起公開)",
            "員工薪資平均數(仟元/人)(每年6/2起公開)",
            "非擔任主管職務之全時員工薪資平均數(仟元/人)(每年7/1起公開)",
            "非擔任主管職務之全時員工薪資中位數(仟元/人)(每年7/1起公開)",
            "管理職女性主管占比",
            "職業災害人數及比率-人數",
            "職業災害人數及比率-比率",
            "職業災害-類別",
        ...
        for column in ["職業災害人數及比率-人數", "火災-件數", "火災-死傷人數"]:
            if column in df.columns:
        ...
        for _, row in df.iterrows():
            # 檢查資料是否已存在
            if EmployeeDevelop.objects.filter(
                year=row["年份"], company_code=row["公司代號"]
            ).exists():
                continue  # 如果資料已存在，跳過此行
            ...
            EmployeeDevelop.objects.create(
                ...
                company_code=row["公司代號"],
                company_name=row["公司名稱"],
                employee_benefits_avg=row.get(
                    "員工福利平均數(仟元/人)(每年6/2起公開)", None
                ),
                employee_salary_avg=row.get(
                    "員工薪資平均數(仟元/人)(每年6/2起公開)", None
                ),
...
def upload_investor_communication_data(request):
    ...
    try:
        ...
        # 清理數據欄位
        for column in ["公司年度召開法說會次數(次)"]:
            if column in df.columns:
                df[column] = df[column].apply(clean_value)

        # 清理網址欄位
        df["利害關係人或公司治理專區連結"] = df["利害關係人或公司治理專區連結"].apply(
        ...
        for _, row in df.iterrows():
            # 檢查資料是否已存在
            if CompanyGovernance.objects.filter(
                year=row["年份"], company_code=row["公司代號"]
            ).exists():
                continue  # 如果資料已存在，跳過此行
...
def upload_sustainability_report_data(request):
    ...
    file_path = os.path.abspath(
        os.path.join(settings.BASE_DIR, "..", "2021-2023_report", "report.xlsx")
    )
...
def load_csv_to_database(request):
    ...
    csv_file_path = os.path.abspath(
        os.path.join(settings.BASE_DIR, "..", "result", "Water_risk.csv")
    )
...
def load_csv_to_database_energy(request):
    ...
    csv_file_path = os.path.abspath(
        os.path.join(settings.BASE_DIR, "..", "result", "energy_risk.csv")
    )
...
def load_csv_to_database_waste(request):
    ...
    # 使用 os.path.abspath 將檔案路徑轉為絕對路徑
    csv_file_path = os.path.abspath(
        os.path.join(settings.BASE_DIR, "..", "result", "waste_risk.csv")
    )
...
def load_csv_to_database_green(request):
    # 使用 os.path.abspath 將檔案路徑轉為絕對路徑
    csv_file_path = os.path.abspath(
        os.path.join(settings.BASE_DIR, "..", "result", "green_risk.csv")
    )
